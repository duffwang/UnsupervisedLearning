...
fig, ax = plt.subplots()
ax.bar(list(range(1,31)), pca.explained_variance_ratio_, linewidth=2, color = 'blue')
plt.axis('tight')
...
means_init = np.array([X_transformed_f[y == i].mean(axis=0) for i in range(1)])

#clustering experiments
#clustering experiments
print("Expected Maximization")
component_list, array_aic, array_bic, array_homo_1, array_comp_1, array_sil_1, array_avg_log = em(X_train_transformed, X_test_transformed, y_train, y_test, init_means = means_init, component_list = [3,4,5,6,7,8,9,10,11], num_class = 7, toshow =0, file_no = "phish_pca")

print("KMeans")
component_list, array_homo_2, array_comp_2, array_sil_2, array_var = kmeans(X_train_transformed, X_test_transformed, y_train, y_test, init_means = means_init, component_list = [3,4,5,6,7,8,9,10,11], num_class = 7, toshow =0, file_no = "phish_pca")
...
data_em_pca_phish = np.concatenate((component_list, array_aic, array_bic, array_homo_1, array_comp_1, array_sil_1, array_avg_log), axis =1)

data_km_pca_phish = np.concatenate((component_list, array_homo_2, array_sil_2, array_var), axis =1)

reconstruction_error_pca_phish = np.concatenate((np.arange(1,31).reshape(-1,1), reconstruction_error), axis = 1)

file = './data/data_em_pca_phish.csv'
with open(file, 'w', newline = '') as output:
	writer = csv.writer(output, delimiter=',')
	writer.writerows(data_em_pca_phish)

file = './data/data_km_pca_phish.csv'
with open(file, 'w', newline = '') as output:
	writer = csv.writer(output, delimiter=',')
	writer.writerows(data_km_pca_phish)

file = './data/reconstruction_error_pca_phish.csv'
with open(file, 'w', newline = '') as output:
	writer = csv.writer(output, delimiter=',')
	writer.writerows(reconstruction_error_pca_phish)
